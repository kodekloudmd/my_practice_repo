...
## 5. Create Topics
```bash
kubectl exec -it kafka-0 -n kafka -- /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --topic raw-email --partitions 4 --replication-factor 2
kubectl exec -it kafka-0 -n kafka -- /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --topic enriched-email --partitions 4 --replication-factor 2
```
...
```bash
kubectl exec -it kafka-0 -n kafka -- /opt/kafka/bin/kafka-console-producer.sh --bootstrap-server localhost:9092 --topic raw-email
kubectl exec -it kafka-0 -n kafka -- /opt/kafka/bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic raw-email --from-beginning
```
